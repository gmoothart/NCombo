...
All files served by NCombo must be in a common root directory. The default is ~/yui/, but this can be customized with the "yuiDir" configuration setting. Similarly, the cacheing directory is ~/yui/cache/ by default, but can be customized.
...
* Wrap cacheing code in its own provider to facilitate unit-tests and extensibility.
